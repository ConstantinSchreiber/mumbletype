...
A macOS-only menubar app that provides global voice-to-text input using OpenAI transcription models. Press the record hotkey (default ⌃D, configurable) anywhere to record audio, press it again to transcribe via the OpenAI API and paste at the cursor position using clipboard + ⌘V.
