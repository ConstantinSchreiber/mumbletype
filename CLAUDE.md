...
- **File transcription language**: config key `file_language` (Preferences popup; "" = auto). Pinning matters: per-chunk auto-detection has translated German recordings into English mid-file
...
- **filetranscribe.py** — Audio-file transcription: two concurrent passes per audio piece. `whisper-1` (verbose_json) supplies the TEXT — the diarize model drifts into translating foreign speech to English even with `language` pinned, and only whisper has segment timestamps; `gpt-4o-transcribe-diarize` (`diarized_json` + `chunking_strategy="auto"`) supplies the speaker timeline. Whisper sentences are attributed to speakers by timestamp overlap (which also absorbs sub-second backchannel interjections). Labels renormalized A, B, C… (the API emits stray labels like `@`); single-speaker collapses to plain text. The diarize model rejects audio > 1400s AND processes at roughly real time, so long files are decoded to PCM (`afconvert`), split into 10-min chunks at quiet points, and transcribed with chunk 1 first (its diarized segments yield 2-10s `known_speaker_references` data-URL clips) then the rest in parallel (3 workers) — the refs keep labels consistent across chunks. Chunk 1's raw→canonical label map is computed once and reused at stitch time (re-mapping would split every speaker in two). Files over the 25 MB API cap or in non-native formats are re-encoded (mono 32 kbps AAC). Falls back to the plain dictation model if the API rejects the diarize model (whisper text still wins when available); 900s per-request timeout override. `App._transcribe_files` puts results in history (kind="file") + clipboard (plain copy, never auto-paste) + `<name>.transcript.txt` beside the source.
